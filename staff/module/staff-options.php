...
if (isset($requestType)) {
    switch($requestType) {
        case "change-staff": {
            ...
            if (intval($sqlStaffAuthor['status']) > 3) {
                ...
                $_SESSION['returnMessage'] = "problem";
                header("Location:../module/gestion-staffs");
                exit;
            }
        ...
        case "searchArticle" : {
            $searchUsername = htmlspecialchars($_POST['articleAuthor'], ENT_QUOTES);
        
            // On prépare la requête Sql
            $sqlGetArticle = $bdd->prepare("SELECT user_id_author AS id_author,articles.id_article AS 'Id', title AS 'Titre',username_author AS 'Auteur',
            DATE_FORMAT(articles.date, '%d/%m/%Y') AS 'Date',COUNT(articles_likes.id_article) AS 'Nombre de likes'
            FROM articles LEFT JOIN articles_likes on articles.id_article = articles_likes.id_article WHERE username_author = ?
            GROUP BY articles.id_article ORDER BY articles.id_article DESC LIMIT 1;");
            $sqlGetArticle->execute([$searchUsername]);
        ...
        case "staffLogSearch": {
            $searchStaffLog = htmlspecialchars($jsonPostData["logToSearch"], ENT_QUOTES); // on évite la XSS en encodant les balises en caractères
            $whatToSearch = $jsonPostData["typeToSearch"]; // on désigne ce qu'on cherche grâce aux données envoyées
            // En fonction de ce que l'on cherche, la requête Sql n'est pas la même, soit on cherche avec username, soit avec id_log
            if ($whatToSearch === "username") {
            ...
            $sqlFindStaffLog->execute([$searchStaffLog]); // On exec
            $staffLogArray = $sqlFindStaffLog->fetch(PDO::FETCH_ASSOC); // On récupère en associant les valeurs
            ...
            if (isset($staffLogArray) && !empty($staffLogArray)) {
                ...
                // On traduit les clés en français puis on renvoie les données à la page en question
                echo "<div class='list-group rounded-2'>";
        ...
        case "changeCaptcha": {
            if (isset($_FILES) && !empty($_FILES['newCaptchaImage'])) {
                ...
                // On récupère le type d'image donc l'extension
                ...
                // On crée le lien de stockage de l'image dans le CDN
                $uploadOk = 1;
                ...
                if (!in_array($_FILES['newCaptchaImage']['type'], $acceptable)) {
                    $_SESSION['post-try'] = "Type du fichier inccorect";
                    header("Location: https://staff.larche.ovh/index");
                    exit;
                }
        
                // On vérifie le format du fichier avec son extension
                ...
                if ($_FILES["newCaptchaImage"]["size"] > 500000) {
                    $_GET['errorUpload'] = "La taille du fichier est trop importante";
                    $uploadOk = 0;
                ...
                if ($uploadOk == 0) {
                    ...
                    // Si tout est ok on envoit le fichier
                    if (move_uploaded_file($_FILES["newCaptchaImage"]["tmp_name"], $file)) {
                        ...
                        $sqlInsertImage->execute([$imageName, $file, $numImage]);
                        header("Location: ../index");
                        exit;
        ...
        case "createChat": {
            ...
            $categorys = array("animals", "jardinerie", "autre");
...
if (isset($_GET) && !empty($_GET)) {
        ...
        case "searchEvent": {
            ...
            if (is_numeric($eventId) !== true) {
                ...
                if ($event !== false && isset($event['Titre'])) {
                    ...
                    <form action='../includes/delete-row.php' method='post'>
    ...
    if (isset($_GET['requestType']) && $_GET['requestType'] === "exportPdf") {
        ...
        if (intval($sqlCheckStaff['status']) >= 3) {
            $sqlGetUserData = $bdd->prepare("SELECT username AS 'pseudo', user.email, name AS 'prénom', 
                lastname AS 'nom', status AS 'rang', user.ip,
                zip_code AS 'code postal', city AS 'ville', age AS 'âge', date_created 'inscription', description, newsletter_pref 'préférence newsletter',
                COUNT(DISTINCT logs.id_log) AS 'nb_login', COUNT(DISTINCT historic.id_historic) AS 'nb_pages'
                FROM user
